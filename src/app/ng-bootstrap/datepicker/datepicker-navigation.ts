...
@Component({
  ...
  template: `
    <button type="button" class="btn-link" (click)="doNavigate(navigation.PREV)" [disabled]="prevDisabled()">
      <span class="ngb-dp-navigation-chevron {{translate.currentLang == 'ar' ? 'right' : 'left'}}"></span>    
    </button>
    ...
    <button type="button" class="btn-link" (click)="doNavigate(navigation.NEXT)" [disabled]="nextDisabled()">
      <span class="ngb-dp-navigation-chevron {{translate.currentLang == 'ar' ? 'left' : 'right'}}"></span>
    </button>
  `
})
