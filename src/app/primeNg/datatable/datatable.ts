...
export class ColumnHeaders {
    ...
    constructor(@Inject(forwardRef(() => DataTable)) public dt: any) { }
...
@Component({
    ...
    template: `
        ...
        <tr *ngIf="dt.isEmpty()" class="ui-widget-content">
            <td [attr.colspan]="dt.visibleColumns().length" class="ui-datatable-emptymessage">{{'NODATA' | translate }}</td>
        </tr>
    `
})
...
export class ScrollableView implements AfterViewInit, AfterViewChecked, OnDestroy {
    ...
    constructor(@Inject(forwardRef(() => DataTable)) public dt: any, public domHandler: DomHandler, public el: ElementRef, public renderer: Renderer) { }
...
@Component({
    ...
    template: `
        <div [ngStyle]="style" [class]="styleClass" [style.width]="containerWidth"
            ...
            <p-paginator [lang]="translate.currentLang" [rows]="rows" [first]="first" [totalRecords]="totalRecords" [pageLinkSize]="pageLinks" styleClass="ui-paginator-bottom"
                (onPageChange)="paginate($event)" [rowsPerPageOptions]="rowsPerPageOptions" *ngIf="paginator && paginatorPosition!='bottom' || paginatorPosition =='both'">gfgfgfhgfggg</p-paginator>
            ...
            <ng-template [ngIf]="scrollable">
                <div class="ui-datatable-scrollable-wrapper ui-helper-clearfix" [ngClass]="{'max-height':scrollHeight}">
                    <div *ngIf="frozenColumns" [pScrollableView]="frozenColumns" frozen="true" 
                        [ngStyle]="{'width':this.frozenWidth}" style="overflow: scroll !important;" class="ui-datatable-scrollable-view ui-datatable-frozen-view"></div>
                    <div [pScrollableView]="scrollableColumns" style="overflow: scroll !important;" [ngStyle]="{'width':this.unfrozenWidth, 'left': this.frozenWidth}"
...
export class DataTable implements AfterViewChecked, AfterViewInit, AfterContentInit, OnInit, DoCheck, OnDestroy, BlockableUI {
    ...
    constructor(public el: ElementRef, public domHandler: DomHandler, public differs: IterableDiffers,
        ...
        public objectUtils: ObjectUtils, public translate: TranslateService) {
    }
    ...
    handleDataChange() {
        ...
        //se if (!this.lazy && (this.sortField || this.multiSortMeta)) {  orginal code
    ...
    handleRowClick(event, rowData) {
        let targetNode = event.target.nodeName;
        if (targetNode == 'TD' || (this.domHandler.hasClass(event.target, 'ac-icon')) || (targetNode == 'SPAN' && !this.domHandler.hasClass(event.target, 'ui-c'))) {
    ...
    highlight() {
        ...
        var el = this.el.nativeElement.parentElement;
        for (var i = 0; i < 10; i++) {
            if (el['parentElement']) {
                el = el['parentElement']
            }
...
@NgModule({
    imports: [CommonModule, PrimeNgSharedModule, PaginatorModule, FormsModule, HttpClientModule,
    ...
    exports: [DataTable, PrimeNgSharedModule],
