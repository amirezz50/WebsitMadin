...
@Component({
  ...
  template: `
  <div #ngxcarousel class="ngxcarousel">
    <div #forTouch class="ngxcarousel-inner">
      <div #ngxitems class="ngxcarousel-items">
        <ng-content select="[NgxCarouselItem]"></ng-content>
        </div>
        <div style="clear: both"></div>
      </div>
      <ng-content select="[NgxCarouselPrev]"></ng-content>
      <ng-content select="[NgxCarouselNext]"></ng-content>
    </div>
    <div #points *ngIf="userData.point.visible">
      <ul class="ngxcarouselPoint">
        <li #pointInner *ngFor="let i of pointNumbers; let i=index" [class.active]="i==pointers" (click)="moveTo(i)"></li>
      </ul>
    </div>`,
  styles: [
    `
...
export class NgxCarouselComponent
  ...
  @ContentChild(NgxCarouselNextDirective, { read: ElementRef, static: true })
  private next: ElementRef;
  @ContentChild(NgxCarouselPrevDirective, { read: ElementRef, static: true })
  private prev: ElementRef;

  @ViewChild('ngxcarousel', { read: ElementRef, static: true })
  private carouselMain1: ElementRef;
  @ViewChild('ngxitems', { read: ElementRef, static: true })
  private carouselInner1: ElementRef;
  @ViewChild('main', { read: ElementRef, static: true })
  private carousel1: ElementRef;
  @ViewChild('points', { read: ElementRef, static: true })
  private pointMain: ElementRef;
  @ViewChild('forTouch', { read: ElementRef, static: true })
  ...
  ngAfterViewInit() {
    if (this.userData.point.pointStyles) {
      ...
      if (this.pointMain && this.pointMain.nativeElement)
  ...
  private carouselPoint(): void {
    ...
    this.pointIndex = Math.ceil(Nos / (!this.data.slideItems ? 1 : this.data.slideItems));
    const pointers = [];
